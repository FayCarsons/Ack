...
let rand_es n max =
  List.init n ~f:(fun _ -> Random.float max, Random.float max, Random.float max)
;;
...
let test_mem _ =
  ...
  let t = O.insert (O.empty test_domain 4) target in
...
let test_map _ =
  ...
  let t = O.load (O.empty test_domain 32) es in
  let t = O.map (fun (x, _, _) -> tuple_splat @@ Num.succ x) t in
  O.iter (fun (x, _, _) -> assert' Float.(1. <= x && x <= 1_000.)) t
;;
...
let test_iter _ =
  let es = List.init 1_000 ~f:(float_of_int >> tuple_splat) in
  let t = O.load (O.empty test_domain 32) es in
  O.iter (fun (x, _, _) -> assert (Float.(0. <= x && x <= 1_000.))) t
;;

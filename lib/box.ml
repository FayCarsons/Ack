...
module Make (Num : Scalar) (Point : Point with type n = Num.t) :
  ...
  let intersects a b =
    ...
    in
    overlap a.min.x a.max.x b.min.x b.max.x && overlap a.min.y a.max.y b.min.y b.max.y
  ;;
end
...
module Make3D (Num : Scalar) (Point : Point3D with type n = Num.t) :
  ...
  let split ({ min; max } as box') =
    ...
    let mid = midpoint box' in
    (* Define the octants *)
    let front_top_left = box (point min.x min.y mid.z) (point mid.x mid.y max.z) in
    let front_top_right = box (point mid.x min.y mid.z) (point max.x mid.y max.z) in
    let front_bottom_left = box (point min.x mid.y mid.z) (point mid.x max.y max.z) in
    let front_bottom_right = box mid max in
    let back_top_left = box min (point mid.x mid.y mid.z) in
    let back_top_right = box (point mid.x min.y min.z) (point max.x mid.y mid.z) in
    let back_bottom_left = box (point min.x mid.y min.z) (point mid.x max.y mid.z) in
    let back_bottom_right = box (point mid.x mid.y min.z) (point max.x max.y mid.z) in
    [| front_top_left
     ; front_top_right
     ; front_bottom_left
     ; front_bottom_right
  ...
  let contains { min; max } ({ x; y; z } : point) =
    in_range (min.x, max.x) x && in_range (min.y, max.y) y && in_range (min.z, max.z) z
  ;;
